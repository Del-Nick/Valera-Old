...
async def message_handler(message: Message):
    ...
    #     change_action(id, 'Start')
        ...
        if 'Расписание' in message.text:
            ...
            answer_is_found = True
            await message.answer('Поздравляем с окончанием сессии!', keyboard=standard_keyboard(id))
            # if len(check_group(id).split(', ')) > 1:
            #     await message.answer('Выбери номер группы', keyboard=group_keyboard(id))
            ...
            #                 date_str = f'{session[key]["Date"][:5]} {session[key]["Time"]}'
            ...
            #                     check_mark_sticker = '&#9989;'
            #
            #                 text = f'{check_mark_sticker}{key}\n\n' \
            #                        f'&#128198; Дата:  {session[key]["Date"]}\n' \
            #                        f'&#8986; Время: {session[key]["Time"]}\n' \
            #                        f'&#128205; Место: {session[key]["Place"]}'
        ...
        with open('Session_Schedule.json', 'r', encoding='utf-8') as f:
            ...
            try:
                ...
                await message.answer(
                    'Не могу найти группу в расписании. Пожалуйста, ответь в свободной форме, '
                    'а потом позови человека, чтобы мы исправили ошибку')
                answer_is_found = False
